...
interface IChipsProps extends Omit<HTMLAttributes<HTMLDivElement>, 'color'> {}

export const Chips = ({ children, className, ...props }: IChipsProps) => {
  ...
  return (
    <>
      <div className={classes} {...events} ref={ref} {...props}>
        {children}
      </div>
    </>
  );
...
export const Chip = memo(
  ({ className, item, onChipClick, size = 'md' }: IChipProps) => {
    const classes = getChipClasses(item.isActive, size, className);
    return (
      <div className={classes} onClick={onChipClick} role="button" aria-pressed={item.isActive}>
        <div>{item.text}</div>
      </div>
    );
  },
  (prevProps, nextProps) => {
    return prevProps.item.isActive === nextProps.item.isActive;
  }
);
